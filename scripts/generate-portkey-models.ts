...
function determineApiType(provider: string, model: string): ApiType {
	if (provider === "vertex-ai" && model.startsWith("anthropic.")) return "anthropic-messages";
...
function normalizeProviderForPricing(provider: string, model: string) {
	if (provider === "vertex-ai" && model.startsWith("anthropic.")) return "anthropic";
...
function normalizeModelForPricing(model: string): string {
	...
	return model.replace(/^anthropic\./, "").replace(/@/, "-").replace(/@default$/, "");
}

async function fetchPricing(provider: string, model: string, canonicalSlug?: string): Promise<PortkeyPricing | null> {
...
const reasoningPatterns = [/o1/, /o3/, /o4/, /thinking/, /reasoner/, /deepseek-r1/, /claude.*(opus|sonnet)/, /gemini.*2\.5/, /gpt-5/];
const imagePatterns = [/vision/, /gpt-4/, /gpt-5/, /claude/, /gemini/];

function inferModelCapabilities(modelId: string, pricing: PortkeyPricing | null) {
	const id = modelId.toLowerCase();
	const reasoning = reasoningPatterns.some(p => p.test(id));
	const supportsImage = imagePatterns.some(p => p.test(id)) || pricing?.type?.supported?.some(s => s.type === "image");
	const input: ("text" | "image")[] = supportsImage ? ["text", "image"] : ["text"];

	let contextWindow = 128000, maxTokens = 8192;
	if (id.includes("claude")) { contextWindow = 200000; maxTokens = id.includes("opus") ? 32000 : 64000; }
	else if (/gemini-2\.[05]/.test(id)) { contextWindow = 1048576; maxTokens = 65536; }
	else if (/gpt-[45]/.test(id)) { contextWindow = 128000; maxTokens = 16384; }
	else if (id.includes("deepseek")) { contextWindow = 64000; maxTokens = 8192; }

	return { reasoning, input, contextWindow, maxTokens };
}

const wordMap: Record<string, string> = {
	gpt: "GPT", ai: "AI", pro: "Pro", mini: "Mini", nano: "Nano", preview: "Preview",
	claude: "Claude", gemini: "Gemini", opus: "Opus", sonnet: "Sonnet", haiku: "Haiku",
	flash: "Flash", embedding: "Embedding", ada: "Ada", codex: "Codex", chat: "Chat", text: "Text",
};
...
function formatModelName(slug: string, canonicalSlug: string): string {
	...
	return base.replace(/_/g, " ").split("-").map(word => {
		const lower = word.toLowerCase();
		return wordMap[lower] ?? (/^\d+\.\d+$/.test(word) ? word : word.charAt(0).toUpperCase() + word.slice(1));
...
async function fetchModels(): Promise<PortkeyModel[]> {
	...
	const data: PortkeyModelsResponse = await response.json();
...
async function generateModels() {
	...
	for (const model of models) {
		...
		const pricing = await fetchPricing(parsed.provider, parsed.model, model.canonical_slug);
		...
		const toMillionDollars = (cents: number) => parseFloat((cents * 10_000).toPrecision(10));
		const inputCost = toMillionDollars(pricing?.pay_as_you_go?.request_token?.price ?? 0);
		const outputCost = toMillionDollars(pricing?.pay_as_you_go?.response_token?.price ?? 0);
		const cacheReadCost = toMillionDollars(pricing?.pay_as_you_go?.cache_read_input_token?.price ?? 0);
		const cacheWriteCost = toMillionDollars(pricing?.pay_as_you_go?.cache_write_input_token?.price ?? 0);
...
`;
	...
	const outputPath = join(__dirname, "..","pi-extensions", "portkey-provider", "portkey-models.generated.ts");
